...
#define INPUT "../static/input.txt"
#define MOOD "../static/mood.txt"
#define ANTONYM "../static/antonym.txt"
#define COMMON "../static/common.txt"
#define OUTPUT "../static/output.txt"
...
bool partialMatch(string word1, string word2) {
    ...
    double value = (double)index / max(word1.length(), word2.length());
...
bool searchWithPM(string word1, string word2) {
    if (partialMatch(word1, word2)) {
        return false;
    }
...
void identifyDifficultWords(ofstream & output) {
    ...
    for (string word : inputList) {
        bool found = binary_search(commonList.begin(), commonList.end(), word, searchWithPM);
        if (!found) {
...
void analyzeConflict(ofstream & output) {
    ...
    for (int i = 0; i < inputList.size(); i++) {
        for (int j = 0; j < antonymList.size(); j++) {
            if (oppositeMatch(inputList[i], antonymList[j].first)) {
                inputList[i] = antonymList[j].second;
            }
        }
    }
    ...
    for (string word : inputList) {
        ...
        if (it != antonymList.end()) {
            ...
            for (; it != it_limit; it++) {
                bool yes = binary_search(inputList.begin(), inputList.end(), it->second);
